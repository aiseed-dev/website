...
バグの発見から修正、PRの提出まで、すべて1つのClaude Codeセッションで行った。

1. **バグの発見** — ブログ記事のビルド結果を確認して異常を検出
2. **原因の特定** — `markdown-it-py` のソースを読み、`scanDelims` の flanking delimiter 判定ロジックを分析
3. **修正の実装** — CJK文字を句読点として扱う6行の修正を作成
...
これらを外部に委託すれば、数日から数週間、数十万円のコストがかかる。

Claude Codeは、ブログ記事を書いている最中にバグを見つけ、原因を特定し、修正し、テストし、PRを提出するまでを**1セッションで完了**した。

:::highlight
**SIerに委託に出す必要は、全くなくなりました。**

必要なのは、問題を正確に伝える力と、結果を判断する力だけだ。コードを書く能力は、もう人間側に必要ない。
:::
