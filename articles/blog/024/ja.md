...
ここでライセンスの非対称が効いてくる。買い切りの Office Home & Business 2024 は、商用利用権こそ付くが、単体で動くデスクトップ版で**共同編集のネットワーク機能はない**。複数人で同時に編集したいなら、結局 Microsoft 365 Business のサブスクが要る。しかもそれは**一人分のライセンス**で、**一緒に編集する相手を加えるたびに人数分の課金**が積み上がる。社員ひとりずつを経費で持てる会社ならまだ回るが、**町内会・PTA・NPO・サークル・勉強会・ボランティア・家族のような「ただのグループ」では、コストを理由に共同編集そのものを諦める**ことになる。ONLYOFFICE DocSpace Community なら、自分のサーバーにみんなをただ招くだけ ── **人数で課金されない**(2026年5月の v9.4 で同時接続数の制限も撤廃された)。会社の外で自由に協働する人たちにこそ、効く。
