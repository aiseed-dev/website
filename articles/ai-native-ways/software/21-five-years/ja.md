...
- **SIer 委託モデルが構造的に不経済** (3-01) ── 同じ手間で自分で
  作れる
- **価格差が桁違い** (3-02) ── 10倍〜100倍、競争ではなく市場破壊
- **ロックインが解消する** (3-03) ── AI ネイティブな標準コード、
  Palantir FDE の対極
- **各社がビルダーを雇用する** (3-04) ── 弁護士・医師と同位置の
  専門職。供給源は元コーダーだけでなく、AI + Python + Flet で
  参入する **VB/VBA 世代・工作派・現場技術者・学生** にも広がる
- **多重下請けが転換を緩衝する** (3-05) ── 雇用調整なしに縮小可能
...
flowchart LR
  ...
  Builder["ビルダー需要<br/>(1-04・3-04)"]
  Customer["顧客自製<br/>(1-05)"]
  SIer["SIer 縮小<br/>(3-01・3-02)"]
  Lock["ロックイン解消<br/>(3-03)"]
  Industry["業界転換<br/>(3-05)"]
...
- 3-05に **「ソフトウェアより物が不足する時代」** セクションを追加
...
- **既存契約の満了サイクル** ── SIer の長期保守契約は典型的に
  3〜5 年。次の更新タイミングで置き換え評価される(3-03)
...
- SIer が一度多重下請けを縮小すると、また下請けを大量採用しない
  (3-05) ── 縮小した契約関係は再構築されない
- ビルダーが専門職として認知されると、その役割定義は持続する
  (3-04) ── 弁護士・医師の位置に動いたものは戻らない
...
4. **判断中心の専門職は本質的に独立志向** ── 弁護士・医師・会計士
   が個人事務所やパートナーシップを好むのは偶然ではない(3-04)
...
(3-01)、顧客が自分で作る(1-05)、判断中心のビルダー(1-04・3-04)、
...
3-04で扱った「**工作派・現場技術者が組み込みに入る**」は、
この中間層への新規参入そのものだった。3-05で見た「**物が不足
...
| 自由都市・ギルド | フィレンツェ、ヴェネツィア、職人組合 | AI 時代の自由人、専門職組合(3-04) |
...
動く**(3-05)。AI データセンター建設、製造業の復活、自然農法
...
- [3-05: 日本のSIer業界の転換と雇用流動性](/ai-native-ways/software/japan-transition/)
