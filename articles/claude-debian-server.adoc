...
本編のlink:/claude-debian/prologue/[序章]で、「AI時代にLinuxの長所のコストが劇的に下がった」と書いた。`journalctl` の読み方をClaudeに聞けば数秒で分かる、という話だ。サーバーでは、この効き目がさらに強くなる。なぜなら、サーバーには「GUIで何となく操作する」という逃げ道が最初からないからだ。すべてが、Claudeに見せられるテキストで完結する。
...
In the main edition's link:/en/claude-debian/prologue/[prologue], we wrote that "in the AI era, the cost of Linux's strengths dropped dramatically" — that you can learn how to read `journalctl` in seconds by asking Claude. On a server, this effect grows even stronger, because a server has no escape hatch of "vaguely clicking around in a GUI" to begin with. Everything resolves into text you can show Claude.
