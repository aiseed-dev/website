...
第4章で、ビルダーが 1 人 + AI で動く理由を見た。決めることと作る
ことのあいだに、人と人の同期が挟まらず、一人が AI と対話して全体を
回せるからだ。この構造は、ビルダーが社内の人間である必要を持たない
── **顧客自身がビルダーをやる**ことも、同じ理屈で可能になる。
...
**(1) AI がコードを書き、設計もする**(第1章・第3章)── 「コードを
書ける人を雇わないと作れない」は、もう要らない。Claude Max 月 3 万円
で、世界最上層のコーディング能力に接続できる。
...
最初から手元にある**。翻訳の往復コストがゼロになる。**ビルダーの
基盤はリベラルアーツ**(第4章)── つまり、医師・弁護士・経営者・
研究者など、判断と言語化を仕事の中心にしてきた専門職は、AI を
組めばその基盤の上にビルダーとして直接動ける。コーディング歴は
必須ではない。
...
「顧客が 9 割を作る」と聞くと、9 割ぶんのコードを自分で書くのか、と
身構える。だが、そうではない。**ビルダーの仕事は、ゼロからコードを
書くことよりも先に、実績ある OSS を組み合わせて、ベンダー製品を
置き換えることから始まる**。

会社のソフトウェアの大半は、もともと「作る」ものではなく「買う」もの
だった ── Microsoft 365、Azure、基幹のベンダーパッケージ。その置き換え
先は、すでに世界中で動いている。認証は PocketBase、文書は OnlyOffice、
共有と版管理は Forgejo、データベースは PostgreSQL、分析は DuckDB、AI は
Command A+ ── どれもオープンで、無料で、手元で動く(親シリーズ第14章で、
Microsoft 365 を 11 の OSS に解いた)。

ビルダーがやるのは、**選び、繋ぎ、自社の文脈に合わせる** ことだ。これは
第4章のビルダー定義 ──「何を作るか決め、AI に作らせ、出力を評価し、
構造を統合する」── の、最も具体的な姿である。コードを一行も書かずに進む
部分が、ここには大量にある。

順序にも意味がある。**まず OSS でベンダーを外し、土台(認証・データ・
共有)を自分の側に置く**。その上で、**本当に自社固有のロジックだけ**を、
AI と一緒に書く。コードを書くのは、土台を据えた後半だ ── しかも、書く量は
自社固有の部分に縮む。

これは敷居をさらに下げる。ゼロからの実装より、動いている OSS を組むほうが
易しい。「9 割を顧客自身が」は、コーディング能力ではなく、**置き換えの設計
判断**で決まる ── それは、リベラルアーツの基盤を持つ専門職が最も得意と
するところだ(第4章)。

> 9 割を自分で作る、の大半は「コードを書く」ではない。
> **実績ある OSS を組んで、ベンダーを外す** ことだ。
> コードは、その上に載る最後の薄い層にすぎない。

見落とされがちな逆転が、ここにある。**汎用的な機能は、すでに OSS として
世界で共有されている** ── 認証も、文書も、データベースも、ビデオ会議も、
誰かが作り、公開し、何万人もが鍛えてきた。ビルダーがゼロから書く必要は
ない。だから、効果の大きさは入れ替わる。**注目は AI に集まるが、仕事の
9 割を実際に担うのは、共有された OSS のほうだ ── AI の効果より、OSS の
効果のほうが大きい**。AI は、その上に載る固有の 1 割を速く書く道具に
すぎない。

この非対称が、「9 割を顧客自身が」を支えている。**汎用は OSS で共有し、
固有だけを AI と書く** ── ゼロから全部を書く前提なら、9 割の内製は無理
だった。世界中のビルダーが汎用部分を OSS として持ち寄っているからこそ、
一人 + AI で会社のソフトウェアが回る。
