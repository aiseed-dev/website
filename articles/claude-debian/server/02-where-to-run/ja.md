...
description: 本書の前提は自宅サーバー——データは自宅の機械で自分で管理する。それでも自宅・VPS・大手クラウドの三つを費用と責任の地図にして見比べる。「なぜ自宅か」を数字で納得し、VPSとクラウドの正しい使いどころ（練習台と公開の入口）をClaudeと一緒に押さえる。
date: 2026.06.10
...
cta_text: 本体は自宅、入口は必要になってから。電源・回線・物理故障を誰が持つかで、あなたの責任範囲が変わる。費用と責任の地図を広げ、「自宅サーバー」という前提を自分の数字で納得する。
...
第1章で宣言したとおり、本書の前提は**自宅サーバー**だ。それでもこの章で三つの選択肢を地図にするのは、「なぜ自宅なのか」を費用と責任の数字で自分の言葉にするためであり、VPSとクラウドの正しい使いどころ——練習台と、第9章で扱う「公開の入口」——を知っておくためだ。読み終えたとき、あなたは「自宅に置く」という前提を、根拠を持って自分の決断として言えるようになる。
...
この表で一番大事なのは、一番下の二行だ。**自宅サーバーは「全部の責任が自分」だが「データは物理的に自分の手元」にある。** VPSやクラウドは「物理故障や回線は事業者が面倒を見てくれる」が「データは他人のデータセンターにある」。第1章で宣言したとおり、本書はデータ主権を最優先する——だから本体は自宅に置く。VPSが活きるのは、責任の大半を預けてしまってよい「練習台」と「公開の入口」の役どころだ。
...
>
> 本書の前提（本体は自宅サーバー、データは自分で管理）で始めるにあたり、第一段階の実験機をどう用意するのが私に向いているか勧めてください——手元の古いPCを使う / 中古PCやミニPCを買う / 数日だけVPSを借りて練習する。理由を添えて、複数案があれば長所短所も比べてください。

予算・目的・回線・公開方針の四点を渡すのがコツだ。この四点がそろうと、Claudeは一般論ではなくあなた専用の推奨を返せる。

## 第三節 VPSの使いどころ——練習台と、公開の入口
...
だから本書の前提では、**本体（アプリとデータ）をVPSに置く道は取らない**。VPSの出番は二つに絞られる——「壊しては作り直す」練習台と、第9章で扱う「公開の入口」（通信は中継するが、データは自宅に置いたまま）だ。以下の選び方の軸も、この二つの役どころを前提に読んでほしい。
...
**公開（第9章）の段階になって初めて、入口をどう作るか——自宅にトンネルを掘るか、VPSを入口として借りるか——を選ぶ。** どちらを選んでも、**本体とデータは自宅から動かさない。** ここで第二節のCGNAT問題や第三節のVPSの役どころが効いてくる。公開は重い決断であり、脅威モデル（第5章）を理解してから踏み出すべきものだ。だから入口の決定を公開の段階まで先送りできるこの道筋が、最も安全で学びが深い。

つまり、急いで決めるべきものは何もない。**本体は自宅——これは前提として最初から決まっている。** 実験はLAN内で始め、公開の段になって「入口」だけを決める。この二段構えが、費用と責任のリスクを最小にする。

### Claudeに聞いてみよう④：自分用の二段構え計画を立てる

> 第2章の結論として、「本体とデータは自宅に置いたまま、まず自宅LAN内の実験機で第3〜8章を回し、公開の段階で入口（自宅トンネル／VPS入口）を選ぶ」という二段構えが勧められていました。
>
...
5. 「本体は自宅に置いたまま、入口は公開の段で選ぶ」二段構えの計画を、Claudeと立てた
