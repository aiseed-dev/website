...
- **業務データは Markdown / CSV / JSON** で持つ(Office に依存しない)
...
しかし、楽をすることには代償がある。前述の Copilot 問題、ベンダー
ロックイン、データ漏洩、常時監視、エネルギー消費 ── **これらすべて、
ユーザーが「楽」を選んだ結果として発生する代償**だ。
...
**短期的には楽になるが、長期的には自由が失われる**。

### 同じパターンの繰り返し

これは多くの場面で繰り返されてきた構造だ。

- **クレジットカード**: 現金管理を楽にする → 与信機関への依存と借金リスク
- **ファストフード**: 料理を楽にする → 健康と食文化の喪失
- **SNS**: 人間関係の維持を楽にする → 注意経済への従属と精神的疲労
- **アルゴリズム推薦**: 選ぶことを楽にする → 視野の狭窄、フィルターバブル
- **エージェント化**: 認知労力の節約 → 情報処理の自立、データ主権、エネルギーバランスの喪失

エージェント化は、**この同じパターンの最新形**にすぎない。

### そして、社会全体から多様性が消える

「楽」の代償は、個人だけの問題ではない。

集中化された同じ AI に全員が乗ると、**判断基準が画一化される**。
良いメールの基準、重要な論点の選び方、注目すべき数字 ── これらが
ベンダーの学習データと評価関数に支配される。組織ごと、業界ごと、
地域ごと、文化ごとに固有に育っていた判断の **多様性が失われる**。

社会全体で見たとき、**多様性は強さ**だ。
- ある AI が偏りを起こしたとき、別の判断系統が誤りに気づく
- ある産業の AI が崩れたとき、別の産業の知見が補う
- ある地域の文化的判断が押されたとき、別の地域の文化的判断が立つ

**全員が同じ AI に乗ると、この相互補正が消える**。Mythos 時代の単一
障害点が、技術ではなく「判断の単一化」というかたちで現れる。

エージェント化に乗ることは、個人としての自立だけでなく、**社会全体の
多様性とレジリエンス** を引き換えに「楽」を買う取引だ。第 11 章で詳しく
扱うが、本書が「1 人 + AI」を推す理由はここにある ── 効率化のためでは
なく、**自立と多様性のため**だ。
...
処理させる。本書の道具立て(Markdown / CSV / Python / Claude Code)は、
このサンドボックス原則と最初から整合している。
