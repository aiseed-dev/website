...
| `mirror_site.py` | 既存サイトをブラウザで取得し、静的ファイル束へミラー（SiteSucker 相当。既定は JS 非実行で生 HTML を保存、`--js` でレンダー、`--skip-existing` で差分更新）。 | [mirror_site.md](../docs/manuals/mirror_site.md) |
| `build_creativeweb_archive.py` | 旧 creativeweb.jp（ASP.NET）を `.NET` 抜きで静的化し `/creativeweb/` へ。広告・旧GA除去、aiseed GA注入、旧301を再現。旧ドメインのリダイレクト手順も収録。 | [build_creativeweb_archive.md](../docs/manuals/build_creativeweb_archive.md) |
