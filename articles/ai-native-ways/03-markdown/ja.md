...
> ディスクに毎日 `git push` する。**「いきなり外向きサーバーは
> 怖い」場合**は、LAN の中だけで運用すればよい。**遅かれ早かれ、
> 自分のサーバーが要る ── miniPC 一台が一番楽な入口**。
...
> に置く**。これがソフトウェア開発編 2-05「自分のシステムを自分の側に置く」の Git
> 版だ。
...
**業務データ・契約書・顧客情報・個人ノート ── 中身が外に出てほしく
ないものは、すべてここに置く**。
