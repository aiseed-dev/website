...
description: Word・Excel・PowerPoint は、人が書き人が読む入出力の道具だ ── 中身を置く場所ではない。Office の中にいる限り AI は同僚にならず、自分は「処理する人」のまま。だから docx・xlsx・pptx は「使う」のではなく「通過させる」形式と捉え、中身の主導権を自分の側に取り戻す。Nextcloud のような別の保管アプリは足さず、文書はファイルのまま自分のストレージに置き、認証は2-03の PocketBase、権限は xattr に持たせ、OnlyOffice Docs を編集エンジンだけ組み込む ── 独自の、薄い文書アプリ。高互換の OOXML だから上司にも同僚にも `.docx` のまま渡せる。完成品の DocSpace は Active Directory を呼び戻すので使わない。実装は公開リポジトリ kura。形式は変えず、主導権だけ取り戻す。
date: 2026.07.04
