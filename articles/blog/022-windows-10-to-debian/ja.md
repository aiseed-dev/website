...
共有地（＝動く中古機・入門機）を潰して、領主の指定する高い区画しか残さない ── 中世の囲い込みと同じ構図だ。だが、エンクロージャーには出口がある。Microsoft が囲い込みたいのは「Windows ユーザー」であって、「PC ユーザー」ではない。**ハードウェアそのものは、そのまま Linux に持っていける**。
