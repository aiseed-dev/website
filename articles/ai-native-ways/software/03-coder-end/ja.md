...
description: コーダーを「コードを書くこと自体を仕事の中心に置く役割」と定義する。AIがコードを書けるようになり、この役割の経済的基盤は崩れる。AIはコードを書くだけでなく設計もする ── Opusはコーダー、FableとMythosは設計までするソフトウェアエンジニアだ(両者は設計能力で同水準)。一方で、システムを作り動かす仕事は、コードを書くことよりずっと広い。ハードの調達、関係者との協議、運用、責任、そしてAIとの対話 ── これらは人間に残る。消えるのは役割定義であって、人ではない。
date: 2026.06.15
...
- **Fable / Mythos** ── **ソフトウェアエンジニア**。設計まで踏み込み、
  構造を自分で決められる(両者は設計能力で同じ水準)

設計までできることには、傍証がある。Fable / Mythos の世代では、攻撃者
が Claude を悪用し、**AI が標的への攻撃工程の大半を自律的に遂行した**
事例が公表されている(2025 年、Anthropic が大規模な自動サイバー諜報
キャンペーンとして報告した)。攻撃を自分で進めるには、相手のシステムを
読み解き、段取りを組み、攻め筋の構造を組み立てる力が要る ── それは、
ソフトウェアを設計する力と同じものだ。攻撃にそれが使えるということは、
設計にも使えるということだ。

つまり AI は、コードを書くだけでなく、**設計**にまで入ってくる。
ここで段が一つ上がる。**以前の AI は「コーダーの代わり」だった。
Fable / Mythos の時代には、要件から設計・実装までを担う「SIer の
代わり」ができるようになった**。かつて人手を要した「コードを書く帯」
の市場価値は、ほぼゼロに収束する ── これは労働観ではなく、価格の
話だ。
...
だけ**だった顧客が、いまは自分で作り・動かす側に立つことだ。しかも ──
かつては SIer に発注するだけでも、RFP の作成、業者選定、要件のすり合わせ、
契約交渉と、相当の手間がかかった。**設計までする AI(Fable / Mythos)
の水準では、その「発注する手間」があれば、顧客自身が作り上げてしまう**
(第5章で詳しく扱う)。

> かつては、SIer に**発注するだけ**でも相当の手間がかかった。
> 設計までする AI(Fable / Mythos)の水準では、その手間があれば ──
> 顧客自身が作ってしまう。
