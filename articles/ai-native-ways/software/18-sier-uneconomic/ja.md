...
── 委託は、**判断する側と、作る側を引き裂く**。そして AI 支援コーディング
では、その裂け目に「危険なコード」が入り込む。

これは実験で裏づけられている。ある大学の対照実験では、AI 補助を与え
られた開発者は、与えられなかった開発者より**有意に安全でないコードを
書いた**。しかも厄介なのは、書いた本人が**自分のコードを安全だと誤認
した**ことだ ── 安心感だけが先に来る。初期の実証研究でも、GitHub
Copilot が出力するコードのおよそ**四割**が、何らかの脆弱性を含むと
報告された。

委託したのは「コードを書く」作業のはずだった。だが補完を受け入れる
たび、人は**「これでいいか」という検証の判断まで、一緒に手放す**。
判断と実体が一つの頭の中で出会わなくなる ── これが委託の穴だ。

では、なぜ危険なコードが通るのか。**設計が違う**。GitHub Copilot は
元来、「次の一行を予測する補完エンジン」だ ── 候補を出し、人間に渡して
終わる。検証の輪が閉じない。対して、AI を自ら作る側が出したエージェント
型 ── Anthropic の Claude Code、OpenAI の Codex ── は、**計画 → 実行 →
テストで検証 → 修正**というループを回す。ソフトウェアだけが持つ「無料で
無限に回せる検証器」(3-08)を、ループの中で実際に使う。

だから危険の源は、「公開コードのゴミを学習したから」ではない。**検証を
回すか、人間に丸投げするか**という設計の差だ。補完型は、判断と実体を
引き裂いたまま、危険なコードを「速く・大量に」差し出す。受け入れた瞬間、
検証する者はどこにもいなくなる。

ただし、どのツールも安全を保証しない。エージェント型にも脆弱性は
見つかるし、検証の輪を最終的に閉じるのは人間だ。だが、輪を閉じる場所が
「自分の手元」にあるか、「補完を受け入れた瞬間に消える」かの差は大きい。

これが委託の構造的な穴だ。作ることを外に出した瞬間、判断と実体は、
二度と一つの頭の中で出会わない。SIer 委託も、規模が違うだけで、同じ
構造を持つ。

だから答えは、内製 ── 顧客自身がビルダーになることだ(1-05)。判断と
検証を一人の手元に握り、検証器を自分で回す者だけが、この穴を塞ぐ。
