...
description: AI 能力到達からコーダー消滅、ビルダー需要、SIer 縮小までの変化は連鎖して、5 年前後で主要部分が完了する。一度動いた構造は元に戻らない。ただしこの「完全置換」が起きるのはソフトウェア開発のような検証可能な領域に限られ、自動運転・翻訳・接客のような領域では last 1% に阻まれて完全置換は起きない ── そこは生産性向上の話だ。本サブシリーズの執筆過程そのものが、その証拠でもある。
date: 2026.08.10
label: Software 11
...
ソフトウェア開発で AI が完全置換に向かう理由は、**この領域が正解の
検証可能な領域だから** だ。第1章で触れたとおり:

- コードがコンパイルを通るか / 通らないか
- テストが通るか / 通らないか
- 競技プログラミングの問題が解けるか / 解けないか

すべて **機械的に判定できる**。AI は学習過程で「合っている / 間違って
いる」のフィードバックを大量に受け取れる。だから AI はこの領域で
超人間水準に到達する。

> AI が超人間水準に到達するのは、**正解の検証可能な領域** だ。
> ソフトウェア開発は、その典型例だ。

## 他の AI 応用は、last 1% で詰まる

逆に、**正解の検証が難しい領域** では、AI の進化は同じ速度では起き
ない。よく挙げられる例:

- **自動運転** ── 99% の場面では問題なく走れる。だが、最後の 1%
  ── 予期しない歩行者の動き、悪天候の判定、子供のボール ── が人命
  を左右する。99% を 100% にする難しさが本質
- **翻訳** ── 99% は通じる訳ができる。だが、ニュアンス・文脈・
  文化的含意の 1% が、信頼を失うか得るかを決める
- **接客・受付** ── 定型業務の 99% は自動化できる。だが、例外対応・
  感情的な配慮・組織内の権限判断の 1% が顧客満足を決める

これらの領域では、AI は **生産性向上の道具** として大きな価値を出
す ── 翻訳の下書き、運転支援、定型問い合わせの自動化。だが
**完全置換は起きない**。99% できることと、100% できることのあいだ
に、深い谷がある。
...
本サブシリーズは、この過大評価から距離を置く。**ソフトウェア開発と
いう特殊な領域 ── 正解が機械的に検証できる領域 ── に限って、完全
置換が起きると論じている**。同じ完全置換が他領域で同じ速度で起きる
とは主張しない。

> 99% できることと、100% できることのあいだに、深い谷がある。
> ソフトウェア開発はその谷を越えた領域だ。他の多くの領域はそうでは
> ない。
...
つまり、本サブシリーズの執筆作業そのものが、自動運転・翻訳・接客と
同じ構造を持っていた ── AI が下書きの大半を書き、人間が判断と修正
を握る。**生産性は数倍に上がるが、完全置換は起きない**。
...
**AI が実行能力で人間トップに到達した。これは正解の検証可能な領域
だから起きた。結果として、コーダーという役割は消え、ビルダーが
代わりに立つ。SIer 委託モデルは構造的に維持できず、5 年前後で
業界の主流が AI ネイティブな内製に移る ── 不可逆に。**

ただし、これはソフトウェア開発という特殊な領域の話だ。同じ速度の
完全置換が他領域(自動運転、翻訳、接客)で起きるとは主張しない。
他の領域では、AI は生産性向上の道具として動く ── 完全置換には至ら
ない。
...
**ソフトウェア開発という一領域での構造転換が、5 年前後で完了する
こと。その転換は不可逆であること。そして、この特殊な領域の話を、
他領域に自動的に拡張してはならないこと**。
