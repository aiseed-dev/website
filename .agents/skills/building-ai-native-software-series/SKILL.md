...
| **自立編 ── M365・Copilot・WordPress・基幹・GitHub から自立する** (part 2) | 第1–10章 / 09–18 | Overview/map (1: Microsoft と Google からの自立 — the bundle, the one-to-one mapping, no build steps) + hands-on build guide (2–10): replace the vendor products with OSS, one stack on the previous. "書くのではなく、立てる"; "the effect of OSS > the effect of AI." |
| **転換編 ── 産業構造の帰結** (part 3) | 第1–6章 / 19–24 | The structural consequence: why the SIer-commissioned model is now obsolete. |
...
- `prev_slug` / `next_slug` chain **inside** the sub-series only, across all **24** chapters in global folder order: folder 01's `prev_*` is empty, folder 24's `next_*` is empty. The chain runs continuously across the three 編 (…導入編8 `embedded` → 自立編1 `independence` → 自立編2 `foundation` → … → 自立編10 `ai` → 転換編1 `sier-uneconomic` …). It does not cross into the parent series. Re-run the chain over all folders after adding/moving a chapter.
...
**自立編 ── M365・Copilot・WordPress・基幹システム・GitHub から自立する (part "2", 第1–10章 / folders 09–18)** — 第1章 is the overview/map (no build steps); 第2–10章 are the install-guide register, each standing up one OSS stack, replacing a vendor product. The 自立編 replaces **both Microsoft 365 AND Google Workspace** (the stack is an alternative to both suites).

| 第n章 | folder | title (JA) | slug | stands up (replaces) |
|---|---|---|---|---|
| 1 | `09-independence` | Microsoft と Google から自立する ── 全体像と対応表 | `independence` | **overview/map only — NO build steps.** Both suites are bundled; the one-to-one mapping (M365 + Google Workspace → OSS) per layer, link to each build chapter; cost/operations/基幹-seam framing |
...
|---|---|---|---|---|
| 1 | `19-sier-uneconomic` | SIer委託モデルの構造的不経済 | `sier-uneconomic` | outsourcing overhead exceeds build cost |
| 2 | `20-price-gap` | 価格競争力の桁違いの差 | `price-gap` | 10–100× displacement |
| 3 | `21-lockin` | ロックイン問題 | `lockin` | Palantir FDE archetype |
| 4 | `22-hiring-builders` | 各社がビルダーを雇用する時代 | `hiring-builders` | builders as a professional class |
| 5 | `23-japan-transition` | 日本のSIer業界の転換と雇用流動性 | `japan-transition` | multi-tier subcontracting enables shift |
| 6 | `24-five-years` | 数年で完了する構造転換 | `five-years` | irreversible, ~5 year horizon |
...
- **自立編 tool choices are fixed** (shipped): data PostgreSQL/SQLite/pgvector/DuckDB/Polars · auth PocketBase · code Forgejo+Zed (replaces GitHub) · documents OnlyOffice Docs embedded in PocketBase (no Nextcloud — heavy legacy monolith) · mail Stalwart · meetings Jitsi/Cal.com/BigBlueButton · web **public site = static → Cloudflare Pages (no server); build→verify→deploy separated, no auto-rebuild. Caddy is only the internal-apps reverse proxy (introduced in 自立編 第3章 auth)** · API FastAPI · AI Ollama/vLLM+RAG. Recurring framing: "書くのではなく立てる," "人は Excel(OnlyOffice)・機械は Polars/DuckDB," and "control yours, capability borrowed / 窓は借り金庫は自分に" (mail relay / Cloudflare Pages / frontier-model API).
