...
## 実例: AI エージェントを Python + cron に置き換える

毎日メールを処理する AI エージェント($200/月)を、Python + cron + AI API($1〜5/月)に置き換える。

**手順 1: 何を自動化したいかを言語化する**

「毎朝、未読の問い合わせメールを読んで、緊急度を判定して、Slack に要約を投稿する」。これだけ。

**手順 2: 一度だけ Claude にコードを書かせる(対話モードで)**

```
あなた: IMAP で未読メールを取得して、Claude API で
        「緊急/通常/不要」に分類、緊急のものは Slack に要約を
        投稿する Python を書いて。設定は環境変数で
```
...
def classify_and_post(subject, body):
    msg = ANTHROPIC.messages.create(
        ...
        messages=[{"role": "user", "content":
            f"このメールを「緊急/通常/不要」に分類し、緊急なら 1 行要約と推奨アクションを返して:\n件名: {subject}\n本文: {body[:2000]}"
        }]
    )
...
# IMAP で未読を取得して各メールを classify_and_post に渡す
# (省略: 30 行ほどの IMAP ループ)
```

**手順 3: 人間がレビュー・承認**

「Claude API のモデル名は最新?」「Slack 通知の頻度上限は?」「失敗時のリトライは?」── 30 分のレビューで気になる点を Claude に修正してもらう。**ここが対話モードの肝**。
...
15 分ごとに動く。**1 度コードを書いたら、AI を呼ぶのは「分類が必要なメール 1 通ごとに 1 回」だけ**。

**手順 5: コストを比較**

| 運用形態 | 月額 |
|---|---|
| AI エージェント SaaS(自律モード) | **$200** 以上 |
| 自前 Python + Claude API(分類処理だけ) | **$1〜5** |

200 倍の差。しかも、**自前運用は自分のサーバーで動く、データが外に出ない、止めたい時に止められる**。

**手順 6: ログを毎週レビュー**

```bash
grep "緊急" /var/log/email-agent.log | tail -20
```

過去 1 週間で「緊急」と判定された 20 件を見直す。誤判定があれば、Claude にプロンプトを調整させる。**判定ロジックを「コードに凍結する」ことで、結果が再現可能・検証可能になる**。

自律エージェントなら、毎回違う判断をする。コードに凍結すれば、**毎回同じ判断**。誤りが見つかったら、その 1 箇所を直せば全体が正しくなる。これが「AI を生成器として使う」ということだ。
