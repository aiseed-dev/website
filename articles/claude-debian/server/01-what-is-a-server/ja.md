...
description: サーバーは「自分が触っていない間も働き続ける裏方」だ。本書の前提は自宅サーバー——データは他社のクラウドに預けず、自宅の機械で自分で管理する。GUIを持たないテキストだけの世界はClaudeと一緒に学ぶ方式と最高に相性が良い。その前提と理由を、この章で整える。
date: 2026.06.10
...
1. **サーバーは自宅に置く。** 使わなくなった古いPCやミニPCを、あなたの家で動かす。VPSやクラウドは、練習台と「公開の入口」（第9章）として補助的に使うだけだ。
2. **データは自分で管理する。** 他社のクラウドサービスに預けず、自宅の機械の上のデータベース（第7章）に置き、自分の手でバックアップする（第10章）。
3. **コンテナは使わない。** アプリは「ディレクトリ + venv + systemd」で動かし、権限とsystemdの砂場で隔離する（第8章）。

つまりこの本は「サーバーの一般論」ではなく、**自宅に自分のインフラを建てて、データの主導権を自分の手に戻す**ための本だ。なぜこの前提を選ぶのか——その理由は第二節で詰める。

この章は手順の章ではない。サーバーとは何か、なぜ自宅で自分で持つのか、このサーバー編をどう読むのか——その三つを整える、いわば二度目の序章だ。
