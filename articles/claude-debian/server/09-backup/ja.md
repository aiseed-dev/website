...
description: サーバー本体は使い捨て、データだけが本体だ。OSもコンテナも数十分で作り直せるが、データだけは作り直せない。何を守るかを決め、resticで暗号化バックアップを取り、systemd timerで自動化し、そして復元を実際に演習する——「復元したことのないバックアップは無いのと同じ」という原則を、Claudeと一緒に手で確かめる章。
date: 2026.06.10
label: Claude × Debian サーバー編 09
prev_slug: claude-debian-server-08-publishing
prev_title: 第8章 外の世界に公開する
next_slug: claude-debian-server-10-operations
next_title: 第10章 サーバーを育てる
cta_label: Claudeと学ぶ
...
cta_btn1_link: /claude-debian/server/10-operations/
cta_btn2_text: 第8章に戻る
cta_btn2_link: /claude-debian/server/08-publishing/
---
...
OSは失われる。だが、それは惜しくない。第3章でやった最小インストールは、手順さえ残っていれば数十分でやり直せる。第7章で組んだコンテナも、`compose.yaml` が一枚あれば数分で立ち上がる。**サーバー本体は、思想として最初から使い捨てだ。** 第3章で最小インストールを勧めたのも、第7章でコンテナを勧めたのも、実はこの章への伏線だった——OSもアプリも「作り直せる」ように作ってきた。
...
1. **アプリのデータ**：コンテナのボリューム、`~/data`、データベースの中身など。これが本体中の本体だ。
2. **設定**：`compose.yaml`、第8章で書いた `Caddyfile`、第6章で作った自作の systemd unit、`/etc` 以下に手で加えた変更。
...
**設定（テキストファイル）は、git で管理する。** これは本編[第12章「設定管理の作法」](/claude-debian/12-config-management/)でデスクトップ向けにやったことの、そのままサーバー版だ。`compose.yaml` も `Caddyfile` も自作 unit も、すべてテキストだ。テキストは git が最も得意とする。変更の履歴が残り、「いつ・なぜこの設定にしたか」が後から辿れる。
...
> 〔第7章までで動かしているもの一覧。例：写真サーバー（コンテナ、ボリュームは /srv/photos）、ファイル共有（~/share）、自作のメモアプリ（DBは /srv/notes/db）……〕
>
...
自分の構成を渡すと、「そのコンテナのボリュームは実はここにある」「そのDBは稼働中コピーが危ない」といった、自分では見落としがちな点が表に並ぶ。守るべきものリストは、この章の出発点だ。
...
凝った監視を組むのは第10章に譲るとして、まずは素朴な運用から始めればいい。**週に一度、自分の目で `restic snapshots` を見る。** 一覧の一番上の日付が、ちゃんと「昨日」や「今朝」になっているか確かめる。これだけで、「気付かないうちに止まっていた」を防げる。
...
手順はこうなる。第3章の最小インストールから始め、git に置いた設定（`compose.yaml`、`Caddyfile`、自作 unit）を clone し、restic でデータを書き戻す。これで、本番とほぼ同じサーバーが新しい機械の上に立ち上がるはずだ。

この演習をやり遂げると、二つのことが手に入る。一つは「本当に最初から作り直せる」という確信。もう一つは、再構築の途中で必ず見つかる**「手順に抜けていたもの」**——メモし忘れていた設定、git に入れ忘れていたファイル。それを見つけて埋めておけば、本物の「いざ」のときに慌てずに済む。第10章で扱う「構成のコード化」は、ここに直結する。

### Claudeに聞いてみよう③：復元演習の手順書を作る

> 私は restic で次の構成のバックアップを取っています：
> 守っているデータ：〔パス一覧〕
> 設定の管理：〔git リポジトリに compose.yaml / Caddyfile / 自作 unit を置いている、等〕
...
これで、サーバーが物理的に壊れても、あなたは慌てない。OSもコンテナも作り直せて、データは戻せる。最後の第10章では、この安心感を土台に、サーバーを**長く育てていく**話をする。週に15分の見守りで回す設計、保守のリズム、壊れたときの型、そして「次の一手」——一台のサーバーを、生き物のように手入れして育てる作法だ。
