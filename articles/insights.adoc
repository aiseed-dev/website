...
これは単なるコスト削減ではなく、*ソフトウェア開発の構造そのものが組み替わる*話だ。SIer 委託モデルが数年で完了する不可逆な転換と、それに代わって立つ「ビルダー」(経営判断を下す経営陣＝CIO)の輪郭は、link:/ai-native-software/[AI ネイティブなソフトウェア開発]の転換編で展開している。
...
This is not merely cost reduction; *the structure of software development itself is being reorganized*. The irreversible transition that completes within a few years, and the shape of the "builder" (management that makes business decisions — the CIO) that rises in the SIer's place, are developed across the Shift part of link:/en/ai-native-software/[AI-Native Software Development].
...
これは「効率化」の話ではない。第一次ルネサンス(14〜17世紀)が活版印刷によって本格化したように、*第二次ルネサンス* がLLMによって本格化している局面 ── その入口に我々はいる。詳細な構造論はlink:/ai-native-software/[AI ネイティブなソフトウェア開発]で展開している(導入編、link:/ai-native-software/[ソフトウェア開発編] 3-07「これは第二次ルネサンスの始まりだ」)。
...
This is not a story about "efficiency." Just as the First Renaissance (14th–17th centuries) was brought into full force by the printing press, the *Second Renaissance* is now being brought into full force by the LLM — and we stand at its entrance. The full structural argument lives in the link:/en/ai-native-software/[AI-Native Software Development] (the Introduction part; link:/en/ai-native-software/[Software sub-series] 3-07's "This is the beginning of the Second Renaissance").
...
* Anthropic 自身が「自律モードで走らせるな」と警告
  (`/ai-native-software/ai-delegation/`)
...
* Anthropic itself warns "do not run in autonomous mode"
  (`/en/ai-native-software/ai-delegation/`)
...
これまで30人で作っていたシステムが、1〜3人で作れる。これまで「専門家チームに発注」だった仕事が、「自分で作る」になる。AI ネイティブな開発の作法を持つ個人は、*SIerに発注する顧客から、自ら作るビルダーへ*転換できる。
...
これが「自立」の実装である。全部を自前にすることではない。*鍵(認証)と管理を手元に置き、借りるものは標準の形で借りる* ── 窓は借りても、金庫は自分の側に置く。組織のスイート全体でこれをやり切る手順は、別の連載 link:/ai-native-software/[「AI ネイティブなソフトウェア開発」の自立編]が扱っている。
...
link:/en/ai-native-software/[AI-Native Software Development].
...
「AI ネイティブなソフトウェア開発」の link:/ai-native-software/python/[2-04「処理を書く ── Python と Flet で、自分の道具を持つ」]が、この道筋を扱っている。
...
*「Claudeと一緒に学ぶDebian」シリーズ*が、この移行を Claude との対話で進めるための完全な手引きである。
...
「AI ネイティブなソフトウェア開発」が、この道筋の具体的なツールと作法を扱っている。
...
*選択肢は既に揃っている*。本書シリーズ（insights の構造分析、claude-debian の実践手引き、ai-native-software の開発の作法）は、その選択を支えるための道具立てである。
...
* link:/claude-debian/[Claudeと一緒に学ぶDebian]——OS から日常環境までの移行手引き
* link:/ai-native-software/[AI ネイティブなソフトウェア開発]——AI と共に立てて、自分で動かす作法
...
Step 5: Explore the path to standing as a builder +
  ...
  → The "AI-Native Software Development" series is the guide for this
--
...
link:/en/ai-native-software/python/[2-04, "Write the Logic — Own Your Tools with Python and Flet"] in AI-Native Software Development covers this path.
...
The *"Learning Debian with Claude" series* is the complete guide for advancing this migration through dialogue with Claude.
...
AI-Native Software Development covers the specific tools and practices for this path.
...
*The options are already in place.* This series — the structural analysis in insights, the practical guide in claude-debian, the development practice in ai-native-software — is the toolset to support that choice.
...
* link:/en/claude-debian/[Learning Debian with Claude] — A complete guide to migrating from OS to daily environment
* link:/en/ai-native-software/[AI-Native Software Development] — Standing it up with AI, and running it yourself
