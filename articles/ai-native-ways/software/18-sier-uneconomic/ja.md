...
委託の不経済は、手間とコストだけではない。委託は、**判断する側と
作る側を引き裂く**。そして AI 支援コーディングでは、その裂け目に
「危険なコード」が入り込む。

Copilot は実際に危険なコードを書く。実証がある。
...
原因は学習データにある。Copilot と Codex の差も、つまるところ**何で
学習したか**の差だ。Copilot は、公開された GitHub のコードで学習した。
GitHub には優れたコードもあるが、**ゴミのようなコードも大量にある**。
質の混じったコードで学習すれば、出力にも質の悪いコードが混じる。

そして、**最終決定を下すのは CEO だ**。ゴミの多い GitHub を学習データに
選んだ責任は、Microsoft の CEO、**サティア・ナデラ**にある。GitHub を
買収し、OpenAI に賭けたのも彼だ。その土台の上で、公開 GitHub のコード
が Copilot の学習データになった。**何で学習させるかは、経営判断だ** ──
その判断が、出力の質に出ている。

もし有能な AI 研究者が開発の中心にいれば、ゴミだらけの GitHub をそのまま
学習させはしなかった。**優良なコードを選んで学習させたはずだ**。学習データ
の質が出力を決めるのは、研究者なら誰でも知っている。

Microsoft には、その研究者がいた。**Microsoft Research** は、産業界で最も
権威ある基礎研究所の一つ ── チューリング賞受賞者を擁する場所だった。だが
**ナデラは、その基礎研究を細らせていった**。CEO 就任初年の **2014 年、MSR
シリコンバレー研究所を閉鎖**(約 50 名)。**2023 年には AI 倫理チームを丸ごと
解体**し、OpenAI のモデルを「最速で顧客に届ける」ことを優先した。長期の
研究は、速度に置き換えられた。

自前の一流研究機関を細らせ、AI の中核を OpenAI に委ねた ── **自社の研究を
判断の中心に据えず、削っていったこと、これがナデラの過失だ**。

委託したのは「コードを書く」作業のはずだった。だが補完を受け入れる
たび、人は**「これでいいか」という検証の判断まで、一緒に手放す**。
判断と実体が一つの頭の中で出会わなくなる ── これが委託の穴だ。SIer
委託も、規模が違うだけで、同じ構造を持つ。

だから答えは、内製 ── 顧客自身がビルダーになることだ(1-05)。判断を
一人の手元に握る者だけが、この穴を塞ぐ。
