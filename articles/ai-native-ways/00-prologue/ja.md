...
**個人レベル ── 手間がかかっている Excel と PowerPoint を Python アプリに**

事務職や個人事業主の現場には、長年の運用で複雑化した Excel と
PowerPoint のテンプレートが残っている:
...
- PowerPoint 自動生成 → Claude が **`python-pptx`** でスライドを
  ...
  顧客 100 社ぶんの提案書も、ループで一気に作れる
- 一回限りの手作業がスクリプト化され、来月も来年も使える
- 担当者が辞めても、コードと README が残る ── 「秘伝」が消える

第3章「処理を書く」と第4章「データを持つ」で身につけた作法の応用。
新規に自分で書くスライド(社内勉強会・カンファレンス発表など)は
Marp(Markdown → スライド、第2章)、**データから自動生成する
スライドは python-pptx** ── 用途で使い分ける。
