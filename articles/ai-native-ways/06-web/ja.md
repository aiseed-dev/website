...
**プロトタイプ汚染**(依存パッケージ経由でのコード注入)、**依存パッケージ
の乗っ取り**(メンテナーアカウント侵害)、**ポストインストールスクリプト
での秘密情報の流出**。検証可能な代表事例だけでも、
...
[第10章 AIで物語を検証する](/ai-native-ways/verify-narratives/)
...
postinstall で任意コードが走る npm のような構造はない。**侵害される
表面積が、桁違いに小さい**。
...
- [第3章: 文書を書く ── Markdownという最小の選択](/ai-native-ways/markdown/)
- [第4章: デザインをする ── Mermaid と Claude デザインで作る](/ai-native-ways/design/)
- [ソフトウェア開発編 2-09: API を作る ── FastAPI で基幹のロジックを出す](/ai-native-ways/software/fastapi/)
