...
One thing must be decided as design, up front. **RAG searches only within the
documents the asker can open** — write this into the spec from day one. Feed
...
Two tiers suffice. By default, put into RAG only the **shared knowledge
everyone can read**, prepared in 2-10 — deciding the scope at preparation time
is the most reliable control. If permissioned documents must be searchable,
carry each document's permission on its pgvector rows and **filter the search
by the asker's token.** The sources then come only from documents that person
can open.
