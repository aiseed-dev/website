...
そしてこの達成が起きたのは、競技プログラミングが **正解の検証可能な
領域** だったから、というのが本質だ。コードがコンパイルを通るか、
出力が想定値と一致するか ── すべて機械的に判定できる。AI が超人間
水準に達するのは、こういう **答え合わせができる領域** だ。本サブ
シリーズが「コーダーが消える」と論じるのは、まさにこの性質を持つ
領域に限った話で、同じ速度の完全置換は他の AI 応用領域 (自動運転、
翻訳、接客など) には拡張できない (この境界線は第11章で扱う)。
