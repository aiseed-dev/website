...
## ただし、文脈を明示に保てなければ、逆に崩れる

ここまでの構造変化には、条件が一つある ── **設計・仕様・意図、つまり
文脈を、明示に保ち、現実とすり合わせ続けること**。

この条件を外すと、構造は逆向きに崩れる。文脈を曖昧にしたまま「機能を
足してくれ」とだけ言って、出てきたコードを取り込み続けると:

- ローカルには動く修正が、全体の整合を壊す
- 同じ概念が、複数の場所で別の表現になる
- 抽象層が増え続け、整理されない
- 次に読むとき、AI にも人間にも、何が一次の意図か分からなくなる
- **数ヶ月で、誰にも保守不能なコードベース**ができる

これは「vibe coding」と呼ばれる失敗形だ。その根は、**「コードを読むのが
一次資料」という古い常識**にある ── コードを真実の源とみなす限り、人は
明示の文脈を持たないまま、ただコードを足し続け、やがて腐らせる。AI が
文脈を理解できると言っても、**渡す文脈が曖昧なら、理解しようがない**。
AI が速いほど、文脈を明示にしていない開発の負債は、速く積み上がる。
**一次資料を、コードから設計・仕様・文脈へ移す** ── それが、vibe coding
を防ぐ鍵だ。

だから、作る側の人間の仕事は、むしろ重くなる。ただし、これは **発注
する側(顧客)が考えることではない** ── 文脈を明示に保ち、負債を溜め
ない規律は、**作る側、つまり SIer 的な役割を担う者(本シリーズで
「ビルダー」と呼ぶ作り手)が引き受ける**。何を変えるか、どの不変条件を
守るか、現実と何をすり合わせるかを決め、AI と対話し、責任を取る ──
これは**コードを書く能力ではなく、現実の文脈を読み、方向と責任を持つ
力**だ。第3章・第4章で、「コーダー」と「ビルダー」の違いとして詳述する。

> 文脈を明示に保つ人間 + AI = 保守コストが激減する。
> 文脈を手放した人間 + AI = 保守不能なコードが量産される。
