...
伝え方は素直でいい ── 「Markdown で書いた記事を、HTML のサイトに変換する
ビルドを作って」。あとは、必要な点を足す。指示に含めるとよいのは、次の
ようなことだ。
...
この指示で、Claude が変換スクリプトとテンプレートを書く。気に入らなければ
「一覧を2列に」「日付の形式を変えて」と、出来た物を見ながら直す(第5章の
作り方そのものだ)。一度できれば、あとは記事を書くだけになる。
