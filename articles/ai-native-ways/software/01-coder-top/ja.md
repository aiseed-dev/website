...
subtitle: Codeforces 2700 帯 ── 月額数千円で、世界の最上層に届く
description: AI のコード作成能力は、競技プログラミングの公開レーティングで人間トップクラスに並んだ。鍵となる事実は二つ ── 到達した能力の水準と、その能力に月額数千円で誰でもアクセスできること。この二つから、ソフトウェア開発編の議論が始まる。
date: 2026.06.01
label: Software 01
...
AI モデルにアクセスする方法は、現在こうだ:

- Claude Pro / ChatGPT Plus / Google AI Pro ── **月額 20 ドル前後**
- Claude / OpenAI / Google API ── 従量課金で、1 万トークン数セント
- 多くの IDE 拡張(Cursor、Claude Code、Copilot)が同じモデルを背面で呼ぶ

つまり、**世界の最上層のコーディング能力に、月額数千円で接続できる**。
クレジットカード一枚と、ブラウザ一つあれば、数分後にはアクセスできる。
...
能力が、月額数千円で手に入る**という一点から、すべてが派生する。
本章はその一点を据えるためだけにある。
