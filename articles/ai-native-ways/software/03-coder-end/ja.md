...
設計までできることには傍証がある。Fable / Mythos の世代では、攻撃者が
Claude を悪用し、**AI が攻撃工程の大半を自律的に遂行した**事例が公表
されている(2025 年、Anthropic が自動サイバー諜報キャンペーンとして
報告)。攻撃を自分で進めるには、相手のシステムを読み解き、攻め筋の構造
を組み立てる力が要る ── ソフトウェアを設計する力と同じものだ。

だから段が一つ上がる。**以前の AI は「コーダーの代わり」だった。
Fable / Mythos の時代には、要件から設計・実装までを担う「SIer の
代わり」ができるようになった**。コードを書く帯の市場価値は、ほぼゼロ
に収束する ── 労働観ではなく、価格の話だ。
