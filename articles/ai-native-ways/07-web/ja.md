...
2026 年に起きた事件を一つ挙げる。**SAP の npm 公式パッケージ 4 種が
汚染され、1,100 を超える GitHub リポジトリのデータが流出した**
`[要検証:具体的な日付・パッケージ名]`。SAP は世界最大級のエンタープ
ライズソフトウェア企業の一つ。その公式 npm パッケージが攻撃の踏み台
になった。

これは新しい話ではない。npm レジストリでの **タイポスクワッティング**
(似た名前のパッケージで誤インストールを誘う)、**プロトタイプ汚染**
(依存パッケージ経由でのコード注入)、**依存パッケージの乗っ取り**
(メンテナーアカウント侵害)── 過去にも、

- 2018 年:`event-stream` 事件(暗号通貨ウォレット狙撃)
- 2021 年:`ua-parser-js` 事件
- 2022 年:`colors`、`faker` 事件(意図的破壊)
- 2024 年:`xz-utils` バックドア(npm ではないが類似構造)
