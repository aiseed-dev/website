...
**本格的に仕事として** ── チームで共有する、過去の決定をたどれる
ようにする、レビューを通す、自動化に繋ぐ ── と決めた瞬間、Git の
...
> **初心者の方へ** ── 一人で書いているうちは要りません。
> `git init` でローカルに履歴を残すだけで十分。**チームで共有する・
> 受け渡しの履歴が要る・PR でレビューする** という段になったら、
> このセクションに戻ってきてください。
...
認証)は Claude に頼めばよい ── これも「書く能力ではなく使う能力」
(第1章)の領域だ。

**(2) Codeberg.org を使う** ── セルフホストしない一番楽な道

[Codeberg.org](https://codeberg.org) は **Forgejo を非営利が
ホストしている公共のフォージ**。アカウントを作れば誰でも使える、
無料。**OSS プロジェクトの公開、個人事業の共有レポジトリ**に最適。
GitHub と同じ感覚で使えて、ベンダーは非営利。
...
GitHub を使ってはいけない、という話ではない:

- **公開 OSS プロジェクト** → GitHub の知名度とエコシステムが強い
- **AI を呼びたいだけの個人ノート** → GitHub でも十分
- **業務データ・社内ドキュメント・契約書のような中身** → GitHub
  に置きたくない。**自分のサーバーに Forgejo か、Codeberg**

GitHub は **2018 年に Microsoft の傘下** に入った。Microsoft 365 /
Copilot と同じ「集中化」の文脈にある。**業務の中身を Microsoft の
クラウドに置きたくないなら、Forgejo セルフホストか Codeberg** を
選ぶ。
...
> 両方とも、自分の手元またはコミュニティの非営利のどちらかに置く
> ── これが「自分のシステムを自分の側に置く」の具体形だ(第5章)。
