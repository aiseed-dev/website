...
## A walkthrough: home-garden temperature/moisture sensor at research-grade

ESP32 + DHT22 (temp/humidity) + soil-moisture sensor; acquire and analyze your garden's environment at research-grade.

**Step 1: write the Python logic on PC first**

```python
# detect.py (run on PC first)
import csv

def should_water(temps, moistures):
    """If 24-hour avg moisture below threshold and temp high, recommend water"""
    if len(temps) < 24:
        return False
...
`python3 detect.py` validates in 0.1 seconds. Tune thresholds on PC freely.

**Step 2: ask Claude for the MicroPython version**

```
You: Translate this Python logic to run on ESP32 MicroPython. Include
     DHT22 and analog soil-moisture sensor reads. Measure every 10
     minutes; show result on OLED.
```

Returned MicroPython (50 lines):
...
**Step 3: transfer to ESP32 and verify**
...
The OLED shows "Temp: 28C / Moisture: 25% / Water!". **$35 of parts** running research-grade environmental monitoring at home.

**Step 4: send to cloud for long-term analysis**

```python
# Add: HTTP POST to a server
import urequests
...
Server side, FastAPI + PostgreSQL collects. After a month, analyze with `pandas`:

```bash
cat data.csv | claude -p \
  "From this month of temperature/moisture data, suggest optimal watering times"
```

**"Watering 2 hours after temperature exceeds 28°C maximizes soil-moisture retention"** — a finding equivalent to agricultural-university research, in your hands.

**Step 5: LINE notification "please water" alert**

Add 5 lines hitting the LINE Notify API in Python. **A "moisture is low in the field" notification** arrives on the smartphone. No IoT vendor contract; this runs on your own hardware.
