...
直線的な工程は、一周ごとに**予算の承認・要件定義・契約**を要し、普通は
年単位になる ── このループを回せない。そもそも、**業務システムの開発を、
本当にアジャイルで回せている現場はほとんどない**。「アジャイル」を掲げても、
予算承認と契約のゲートが、ループそのものを禁じるからだ。だが、**AI を使え
ば、このループを高速に回せる**。実装はすぐ
返る。時間がかかるのは、むしろテストのほうだ。**テストは、全部を自動化
できない**。動くかどうかは自動で測れても、「これは本当に欲しかったものか」
「業務に本当に合うか」は、最後は人が確かめるしかない ── この人手の検証
こそ、顧客の判断であり、外に出せない部分だ。それでも、一周は SIer の
一周(予算承認・契約を含め、普通は年単位)より桁違いに短い。試して、業務を組み替えて、
また試す ── 何度でも繰り返せる。これができるのは、AI を手元に持つ内製
だけだ。
