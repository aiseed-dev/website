...
## 9. 配布前提・客層＝デザイナー（方針更新）

配る意志あり（Apple Developer / App Store 実績あり＝署名・公証は確保済み）。
主な客層は **普通の人、特にデザイナー系**で、Cloudflare を試して自分のサイトを持ってもらうのが狙い。

- **形は GUI 一択**。デザイナーは端末を使わないので、単体 Rust バイナリでも CLI のままでは届かない。
  CLI はあっても裏方どまり。
- **v1 スコープ = 「フォルダ→公開→URL」GUI**（本メモ §2 の deploy のみ）。R2 snapshot は後段。
  → 検討が一周し、最初に出た「Flet デスクトップ公開アプリ」が、この目的にちょうど合致した。
- 位置づけの正直な補足: ダッシュボードの drag-and-drop が初回トライアルでは最も楽（トークン不要）。
  ただし **自分のサイトを持ちたいデザイナーには、ローカルから往復なしで公開できる道具は普通に必要**
  （当初の「ダッシュボード対抗」寄りの評価は過小評価だった）。アプリの勝ち筋は反復運用
  （前回フォルダ記憶・トークン保存・ワンクリック・将来はフォルダ監視→自動再公開）。

### 9.1 受託案件 — クライアントのデータ所有（backup の格上げ）

デザイナーが**受託**でサイトを作る場合、主役は **クライアント（持ち主）**。
要件は「**クライアントが自分のデータを所有し、デザイナーにも Cloudflare にも縛られない**」こと。

- よって backup は v2 の任意機能ではなく、**受託では一級の要件（反ロックイン）**。§3 の snapshot を
  「デザイナー自身の保全」だけでなく「**クライアント引き渡し**」用途に格上げする。
- 鍵は **可搬アーカイブ**: 「バックアップをダウンロード」1ボタン → サイト一式が tar.gz/zip 1個で手元に。
  Cloudflare の知識も認証情報も不要で、どこにでも保管できる＝**それ自体が反ロックインの成果物**。
- 境界（正直に）:
  - **アカウント名義** — 真の反ロックインは Pages も R2 も **クライアント名義**に置きデザイナーは運用代行。
    全部デザイナー名義だと縛られる。ツールは「クライアント名義紐付け／エクスポート」を後押しすべき。
  - **静的 vs 動的** — 静的（受託の主流）は「データ＝ソース＋アセット」で可搬アーカイブで完結。
    動的（CMS / D1 / R2 ユーザ投稿 / KV）は backup 対象が増える別問題。まずは静的＝アーカイブ、
    動的は別途、と線を引く。

---

## 10. 3 スタック試作（trial）の結果

配布前提・GUI 必須が固まったので、`apps/cf-publish/{rust,flet,flutter}/` に「フォルダ→Pages 公開」
コアを 3 スタックで試作し、技術選定の材料にした（いずれも既存 `tools/cloudflare_pages_deploy.py` 準拠）。
...
- **暫定推奨**: §9 の通り **v1 は Flet**（Python ロジック流用＋Flutter 描画で最速にデザイナーへ届く）→
