...
この章は、デスクトップ編の[第7章「インストール実行の対話」](/claude-debian/07-installation-execution/)を一から繰り返さない。**UEFIブート、ディスク選択、暗号化パスフレーズ、ユーザー作成、GRUBといった共通部分は本編第7章がそのまま使える。** ここで扱うのは、サーバー特有の「差分」——ソフトウェア選択をどう削るか、デスクトップが無い環境にどうログインするか、IPアドレスをどう固定するか——だけだ。本編第7章を脇に開いて読んでほしい。
