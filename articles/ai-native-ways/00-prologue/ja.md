...
- **Word ファイルで Markdown 化できるものを Markdown に**(第1章)
  ── 既存の `.docx` は Claude / pandoc で Markdown に一括変換、
  自分が新規に書く文書は最初から Markdown で書く。複雑な書式・
  変更履歴・埋め込みオブジェクトを持つ Word は Markdown 化できない
  ので、そのまま OnlyOffice で運用する
...
**個人レベル ── Excel で手間がかかっている処理を Python アプリに**

事務職や個人事業主の現場には、長年の運用で複雑化した Excel が残って
いる:

- 月次集計を毎月手作業で繰り返している
- 複雑な VLOOKUP / INDEX MATCH / ピボットのチェーン
- マクロ込みのテンプレート Excel が部署間で受け渡されている
- 担当者が辞めると操作手順が分からなくなる「秘伝の表」

これらを **AI で Python アプリに置き換える**。Claude に Excel
ファイルを渡して「この処理を Python に書き換えて」と頼むと、入力
(SQLite / OnlyOffice / CSV / Excel)、処理(Polars + 業務ロジック)、
出力(OnlyOffice の `.xlsx` / PDF / Web 表示)が再現される。
一回限りの作業がスクリプト化され、来月も使える。**担当者が辞めても、
コードと README が残る** ── 「秘伝」が消える。
