...
description: 1-05の最初の実例。たいていの会社の足元にある Excel の VBA マクロ ── セキュリティに脆弱で、作者も去り誰も直せない ── を、AI と Python に移す。Python は AI 自身が書かれている言語で、AI が超得意。Polars と openpyxl で Excel(.xlsx)はそのまま読み書き・編集できる(ODF を使う LibreOffice より、OOXML を保つ OnlyOffice が噛み合う)。現行マクロを正解器に、入出力を見比べて一本ずつ。
date: 2026.06.30
...
1-05で見たとおり、個人は OSS と AI で、自分の道具を作れる。最初の実例は、
...
ここで一つ、原則がある。**Excel を CSV に落とすな**。CSV にすると、書式・
数式・セルの体裁が、すべて失われる。**.xlsx のまま扱う** ── 値の集計や変換
は Polars、書式を保ったままの編集は openpyxl で、どちらも CSV を経由せず
.xlsx を直接読み書きする。

もう一点。**LibreOffice や OpenOffice は ODF 形式(.ods)**で保存
するため、Polars や openpyxl では **扱いづらい**。だから、表計算ソフトその
ものを置き換えるなら、Microsoft と同じ **OOXML(.xlsx)** を保つ
**OnlyOffice** を選ぶ ── Python の道具とそのまま噛み合う(2-05の
文書基盤で扱う)。
...
分からない所は、その場で AI に聞く。これは1-05の作り方そのものだ。

なお、**現行マクロを読み解かず、一から作ってもいい**。そのマクロが何を
しているかは、業務をやっている本人なら分かるはずだ ── なら、やりたいこと
を AI に伝えて、対話で作り直せばいい。**現行を正解器にするのも、ゼロから
対話で作るのも、どちらでもいい**。

## まとめ

- **脆弱な VBA は、早急に Python へ** ── セキュリティの観点でも待ったなし
- **Excel はそのまま(.xlsx)** ── CSV に落とさず、Polars・openpyxl で直接
  読み書きする(CSV は書式が失われる)。置き換えるなら、ODF ではなく OOXML
  を保つ OnlyOffice
...
- [2-05: 文書を取り戻す ── OnlyOffice Docs](/ai-native-ways/software/documents/)
