...
- The public-web form (Chapter 7) and the in-house apps call the same API —
...
The API reads and writes the Chapter 1 **PostgreSQL** and verifies identity with
the Chapter 2 **gate (PocketBase)** token. No new foundation — it rides on what
already exists.
...
@app.get("/orders")
def orders(user=Depends(verify_token)):       # the Chapter 2 gate verifies who
    return db.query("SELECT * FROM orders WHERE user_id=%s", [user.id])  # the Chapter 1 DB
```
...
auth + **FastAPI** + a Flet front end. The code lives in the Chapter 3 Forgejo,
called from the Chapter 7 public web and the in-house apps.

## Summary

- Put your own logic into **one API** with FastAPI
- It rides on the **Chapter 1 DB and the Chapter 2 gate** — no new foundation
...
- [Chapter 1: Lay the Foundation — PostgreSQL, SQLite, and more](/en/ai-native-ways/software/foundation/)
