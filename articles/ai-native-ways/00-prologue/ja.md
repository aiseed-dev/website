...
考えは捨てる ── **そもそも型安全は必要ない**。型安全が要るように
見えるのは、**細かい業務処理を人間が大量に手で書こうとするから**
だ。本書では細かい処理は **AI が書く**(序章「核心」)── 型の
食い違いは AI と実行時のテストが即座に直す。コンパイル時型検査の
重みは要らない。何より、**Java や C# は構造型データ(JSON /
YAML / SQLite / Parquet)を素直に扱うのが難しい** ── パッケージ
生態系の厚みが Python と桁違いだ。AI ネイティブな時代において、
**Java や C# はもう過去の言語**。
