...
title: AIは無知だから間違えるのではない ── ある誤答から考える、AIとの付き合い方
subtitle: 返ってくるのは「商業的に加重された世間の平均」── 誤りの機序が分かれば、使い方も定まる
date: 2026.07.21
description: システム間のデータ連携を生成AIに相談したら、「ゼロトラストは運用の首を絞める。データダイオードによる物理隔離が最も堅牢」という答えが返ってきた。もっともらしいが、結論は逆さまだった。AIはこの件について無知だったのではない。「ゼロトラスト」という言葉についてウェブを埋めているベンダー言説の重心 ── 商業的に加重された世間の平均 ── を、そのままなぞったのだ。一つの誤答を解剖して誤りの機序を突き止め、そこから導かれる五つの使い方 ── 一次資料を与える、正典を自分で選ぶ、もっともらしさと正しさを分ける、検証できる領域で使う、反対意見をぶつける ── を整理する。
lang: ja
label: Blog
category: 構造分析ノート
---
...
- ブログ [MicrosoftのCopilotの課題 ― コードの「正しそうで間違っている」問題](https://aiseed.dev/blog/copilot-correct-looking-but-wrong/) ── 「もっともらしさ」と「正しさ」が乖離する、もう一つの事例
- ブログ [AIを使って、対話だけでもアプリを作ることができる](https://aiseed.dev/blog/building-apps-through-dialogue/) ── AI は正しい答えをくれる相手ではなく、速さをくれる道具
- ブログ [Fable 5 が戻ってきたら、すぐやるべきこと ── 稼働中システムを、全部検証する](https://aiseed.dev/blog/verification-shock/) ── 「検証を手放さない」ことの、組織スケールでの裏面
