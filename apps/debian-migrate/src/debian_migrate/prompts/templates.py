...
def hardware_prompt(hw: HardwareInfo) -> str:
    ...
    body = dedent(
        ...
        # 質問
        この構成で Debian 13 (Trixie) をインストールする場合、
        - 推奨されるデスクトップ環境 (GNOME / KDE / XFCE 等) は何か
        - インストール前に決めておくべき設定 (ディスクパーティション・暗号化)
        - Windows 11 ホストの場合の事前作業 (BitLocker / 高速スタートアップの無効化)
        - 既知の落とし穴
        を、ステップで教えてください (本書 第5章 / 第6章 / 第7章 / 第8章 を踏まえて)。
        netinst ISO は non-free firmware を同梱済みである点も前提にしてください。
        """
    ).strip()
