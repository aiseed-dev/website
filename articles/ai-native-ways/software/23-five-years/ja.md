...
- **AI が実行能力で人間トップに到達** (導入編 第1章) ── Codeforces 2700 帯、
  月 3 万円
- **保守の主戦場が設計に移る** (導入編 第2章)
- **コーダーという役割が消える** (導入編 第3章)
- **ビルダーという新しい役割が出現** (導入編 第4章)
- **顧客自身がビルダーをやる** (導入編 第5章) ── 9 割は内製、1 割だけ外注
- **SIer 委託モデルが構造的に不経済** (第1章) ── 同じ手間で自分で
  作れる
- **価格差が桁違い** (第2章) ── 10倍〜100倍、競争ではなく市場破壊
- **ロックインが解消する** (第3章) ── AI ネイティブな標準コード、
  Palantir FDE の対極
- **各社がビルダーを雇用する** (第4章) ── 弁護士・医師と同位置の
  専門職。供給源は元コーダーだけでなく、AI + Python + Flet で
  参入する **VB/VBA 世代・工作派・現場技術者・学生** にも広がる
- **多重下請けが転換を緩衝する** (第5章) ── 雇用調整なしに縮小可能
...
```mermaid
flowchart LR
  AI["AI が実行能力で<br/>人間トップ到達<br/>(導入編 第1章)"]
  Coder["コーダー消滅<br/>(導入編 第3章)"]
  Builder["ビルダー需要<br/>(導入編 第4章・転換編 第4章)"]
  Customer["顧客自製<br/>(導入編 第5章)"]
  SIer["SIer 縮小<br/>(第1章・第2章)"]
  Lock["ロックイン解消<br/>(第3章)"]
  Industry["業界転換<br/>(第5章)"]
...
持つ。これが導入編 第3章「コーダーは消えるがビルダーは残る」の意味だ
...
── 動くが平凡なコードが膨大に生まれ、保守は崩壊する(導入編 第2章の vibe
...
導入編 第4章で触れたとおり、このサブシリーズは 1 人 + AI で 1 週間ほどで
...
- 導入編 第1章に **「IT 革命の成就」** フレーミングを追加
- 多重下請けの起源を **「大量コーダー需要」** と正確に説明
- 第5章に **「ソフトウェアより物が不足する時代」** セクションを追加
...
では **数年で** 起きる。本サブシリーズが描く 5 年前後の構造転換は、
...
| 教養 | リベラルアーツ(自由七科) | リベラルアーツの現代版 ── 判断・言語化・論理・体系的思考・倫理(導入編 第4章) |

中世のリベラルアーツが「教養だけ」では成立しなかったように、
AI 時代のリベラルアーツも単独では成立しない。**四つが揃って
初めて、自由人が成立する**。自由市民がギルド(同業者組合)を結成
して経済力と発言力を強めたように、AI 時代のビルダーも **判断を
売る専門職** として、弁護士会・医師会のような **同業者組合的な
集まり**(第4章)を組む方向に動くだろう。
...
2. **1 人 + AI = 10 人チーム**(導入編 第4章)── 集中化のメリットが消える
3. **判断と実行の境界が一人の中で閉じる**(導入編 第4章)── 集約・調整・
   管理の overhead が純粋な無駄になる
4. **判断中心の専門職は本質的に独立志向** ── 弁護士・医師・会計士
   が個人事務所やパートナーシップを好むのは偶然ではない(第4章)
...
(第1章)、顧客が自分で作る(導入編 第5章)、判断中心のビルダー(導入編 第4章・転換編 第4章)、
...
第4章で扱った「**工作派・現場技術者が組み込みに入る**」は、
この中間層への新規参入そのものだった。第5章で見た「**物が不足
...
その中の特定領域ではない(導入編 第1章・導入編 第3章)。**特化を深めても、
...
|---|---|---|
| 復活する古典 | ギリシャ・ローマの古典学 | リベラルアーツ(導入編 第4章) |
| ポリマス(万能人)の理想 | レオナルド・ダ・ヴィンチ | ビルダー、1 人 + AI(導入編 第4章) |
| 個人の主観性 | 人文主義の「私」 | 自分の道具・自分のデータ・自分の判断 |
| 俗語の解放 | ダンテのイタリア語、ルターのドイツ語 | 自然言語が「プログラミング言語」になる |
| 自由都市・ギルド | フィレンツェ、ヴェネツィア、職人組合 | AI 時代の自由人、専門職組合(第4章) |
| 印刷術による加速 | 1450年代、**読む**ことの民主化 | LLM、**作る**ことの民主化(本章) |
| 改革 | 宗教改革(ローマ教会への反) | 反ベンダー集中、反雇用中心、反 SIer(本書) |
| 新興階級 | ブルジョア(商業・銀行・製造) | AI ネイティブ・ビルダー、自営の判断専門職 |
...
上級ビルダーが全体を把握できない、ロックインを生む(第3章)、
保守困難、判断の連鎖が分散する。中規模の focused なシステムの
組み合わせに分解されていく。

**中規模は上級ビルダーの本領域** ── 判断の連鎖が一人の中で閉じる
規模(導入編 第4章)、弁護士・医師(第4章)と同じ位置。中規模のアプリ
...
動く**(第5章)。AI データセンター建設、製造業の復活、自然農法
...
そして導入編 第4章で名付けた、もう一つの底流 ── **技術職の基盤学問が、
