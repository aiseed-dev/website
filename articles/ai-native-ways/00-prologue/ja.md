...
## 移行の進め方 ── 最初の一歩は順序付き、それ以降は自由

「すぐ消す」と言い切ったが、**現実の移行は一晩では済まない**。
ただし、**順序付けが必要なのは最初の一歩だけ** ── そこさえ越えれば、
自分の業務で「面倒だな」と思うところから自由に手を付けていい。

### 最初にやること ── ベンダーから抜ける(順序が重要)
...
### その後にできること ── 順序は自由、面倒なところから手を付ける

最初のステップで OnlyOffice + ローカル Python の基盤ができれば、
**あとは順序付けの必要は無い**。自分の業務で困っているところ、
面倒なところ、節約したいところから手を付ける。以下、どれから
やってもいい。
...
- **Word ファイルを Markdown + Mermaid に**(第1章・第2章)── 既存
...
手作業からローカルの Python に移すこと」**(第3章)。基幹システムは
組織共有のデータ記録元として残し、人間が触れる UI・レポート・帳票
生成・集計はすべて手元に降ろす。基幹システムとは API か CSV
エクスポートで読み書きするだけ。
...
- **会計 SaaS のサブスクが要らなくなる** ── 請求書発行も上記の
  パターンで完結する(第3章「個人の生産性向上は、基幹システムの
  簡素化につながる」)

新規に自分で書くスライド(社内勉強会・カンファレンス発表)は
Marp(第2章)、**データから自動生成するものは python-pptx /
