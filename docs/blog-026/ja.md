# Fableの公開で業務システムは自営できるようになった

Claude Fable 5の公開(2026年6月9日)に合わせて、Anthropicが一つの事例を発表しました。決済サービスのStripeが、5,000万行のRubyコードの更改(マイグレーション)をFableで実行し、チーム全体で2か月以上かかる作業を、1日で終えた——別の言語への乗り換えではありません。動いているコードを、止めずに、生きたまま書き換えた、という事例です。
...
現行のコードは、読む必要がありません。**コードは、置いていく。**

ひとつ、はっきりさせておきます。冒頭の Stripe は、現行コードを生かしたままの横断移行でした。ここで勧めるのは、現行を正解器にした差分照合での作り直し。向きは逆ですが、どちらも同じ理由で成り立ちます——**Fableが、書いて・確かめて・直すを、機械の速さで回せるからです。**
...
難しいのは、**複数の表にまたがる処理**だけです。受注を書いて、在庫を引いて、売掛を立てる——ここは実装が難しくなります。ただし、画面の数で言えば、ここはシステムのごく一部です。簡単な層で道具に慣れてから向かえば、一人でもできなくはない高さです。
