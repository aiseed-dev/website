...
## 同じやり方で、全領域に ── Linux + Python + AI との協働

道具立ては多く見えるが、**底に共通する作法は一つ**だ:

> **Linux + Python + AI との協働** ── これだけで、
> 文書も、ソフトウェアも、デザインも、組み込みも、
> **デスクワークの殆どの領域を同じやり方で扱える**。
...
### 同僚や部下と同じように対応する ── 信頼して頼む + 結果はきちんとチェック

ここで起きやすい間違いは、二つある:

- **「念のため自分でやっておこう」** と細かい作業を抱え込む
- 逆に **「AI が言ったから」と何も確認せず受け入れる**

どちらも古い癖だ。正しい接し方はシンプル ── **同僚や部下と同じよ
うに対応する**。

> **信頼して頼む + 結果はきちんとチェック** ── これが本書の AI と
> の作法の核心だ。

優秀な同僚や部下に作業を頼むとき、あなたは:

**信頼して頼む** 側:

- **何をして欲しいか明確に伝える**(入力・出力・条件・締切)
- 細かい手順を指示しすぎない ── 任せる範囲を区切ったら、中身は
  本人に任せる
- 「自分でやった方が早い」と引き取らない

**結果はきちんとチェック** する側:

- 出来上がったものを **読んで確認する**(機械的に受け入れない)
- 違うところは **言葉で指摘して直してもらう**
- 何度か往復しても合わなければ、**頼み方を見直す**
- 大事な判断は **自分で下す**

Claude への接し方も、これと同じだ:

- 出来上がりが期待と違っていたら → 言葉で修正を頼む
- エラーが出たら → エラー文を貼って原因と修正を頼む
- 何度往復しても解決しないなら → 自分の頼み方を見直す、または
  設計を見直す合図(第1章「動かなかったを恐れない」)
- 数字・固有名詞・引用・コードの動作 ── **目で確認する**

**これは AI だから特別、ではない**。自分で書いたコードでも、同僚
から受け取った報告書でも、**動作確認・テスト・数字の照合は同じ
ようにきちんとやる**。「自分で書いた=信用できる」とは限らない
(タイプミス、思い込み、計算ミス、コーナーケース見落とし ──
誰でもやる)。**変わるのは「書く部分が AI に渡ること」だけ。
テストの作法は変わらない**。

**「いちいち疑う」のでも、「全部丸呑みする」のでもない**。
**信頼して頼む + 結果はきちんとチェック** ── 同僚との作法そのままだ。

> 確認・テストは、**コードや文書を誰が書いたかとは独立**だ。
> 自分でも、同僚でも、AI でも、やる作業は同じ。

これに **慣れる** ── これが本書の作法の土台だ。第5章で見るように、
Office から離れた瞬間に AI は「道具」から「同僚」に変わる ──
**そう扱う準備が、いまここで始まる**。
