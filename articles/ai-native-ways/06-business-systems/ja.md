...
データベースは残す。50 年前の SQL は、これからも 50 年動く。**SQL の書き換えは、コストに見合わない**。

しかし、データベースの上で動いている Java や C# のロジック層 ── これは書き換える対象だ。Python で書き直して、同じデータベースを叩く。並行稼働させる。

「データの形」と「処理の形」を分ける。データはそのまま、処理は AI ネイティブに。**ここが境界**だ。境界をデータベースに引く。これでデータの整合性を心配せずに、処理層だけ並行稼働できる。
