...
- [pyasciidoc](https://github.com/aiseed-dev/pyasciidoc) — `.adoc` 記事のパーサー（任意。
  `.adoc` を使わないなら不要。未導入で `.adoc` をビルドしようとすると
  分かりやすいエラーになる）
...
**1記事 = 1フォルダ**。同じフォルダ内に `ja.md` / `en.md`（または
`ja.adoc` / `en.adoc`）と関連アセット（画像・PDF）を同居させる。
表・脚注・引用の帰属など CommonMark では足りない構造を持つ記事は
`.adoc` で書く（構文は [pyasciidoc](https://github.com/aiseed-dev/pyasciidoc) の README 参照）。
同じ言語で `.md` と `.adoc` を両方置くと、警告のうえその記事は
スキップされる（どちらが正か推測しない。片方に決めること）。
フロントマターの書式・出力先・Mermaid フェンスの扱いは共通——
`build.markdown.render_body()` が拡張子で振り分ける。
