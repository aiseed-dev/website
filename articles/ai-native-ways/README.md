...
**親シリーズ全 13 章公開済み**(00 序章 + 01〜12 章、JA + EN)。
...
| 01 | `python` | 処理を書く ── AIにPythonで書いてもらう |
| 02 | `markdown` | 文書を書く ── Markdownという最小の選択 |
| 03 | `design` | デザインをする ── Mermaid と Claude デザインで作る |
| 04 | `data-formats` | データを持つ ── JSONとYAMLで考える |
| 05 | `office-replacement` | 事務処理を変える ── Officeから離れる現実的な道筋 |
| 06 | `business-systems` | 業務システムと付き合う ── 並行稼働で書き換える |
| 07 | `web` | Webを作る ── HTML+CSS+JavaScriptという原点回帰 |
| 08 | `apps` | アプリを作る ── CLIツール、Fletアプリ、Flutterアプリ |
| 09 | `embedded` | 組み込みを作る ── Pythonで考え、Claudeに翻訳させる |
| 10 | `ai-delegation` | AIに任せる仕事を見極める |
| 11 | `verify-narratives` | AIで物語を検証する |
| 12 | `one-plus-ai` | 1人+AIで作る、新しい仕事の単位 |
