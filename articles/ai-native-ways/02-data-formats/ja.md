...
「2025年の売上」を集計するつもりが、気づけばセルの配色を整えている。「青は確定、赤は仮、黄色は要確認」── このルールは、ファイル名にも、コメントにも、別ドキュメントにも、どこにも書かれていない。**作った本人の頭の中にだけある**。
...
CSV を Claude に渡して、`matplotlib` で **ビジネスダッシュボード品質のグラフ**を 3 分で生成できる。配色、ラベル、凡例、注釈、トレンドライン ── 統計の専門家レベルの可視化が、データサイエンティストなしで作れる。

JSON データから、**自動的にリアルタイム更新付き Web ダッシュボード**を立ち上げられる。FastAPI + Chart.js + Claude のテンプレで、**売上・在庫・KPI の可視化サイトが 1 日で完成**。BI ツールのライセンス費が要らない。

YAML 設定ファイル一つで、**複雑なシステム全体(DB、Web サーバ、ジョブ、監視、ロードバランサ)が立ち上がる**。Infrastructure as Code の世界 ── 個人開発者が、Google レベルのインフラを設計できる時代だ。

CSV を Claude に投げて「このデータで `pandas` の Jupyter ノートブックを書いて、業界統計と比較分析して」と頼めば、**博士課程の学生が 1 週間かける分析が 30 分で出てくる**。
