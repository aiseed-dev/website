...
piled there, on your own side. The **pgvector** enabled back in Chapter 2
...
**Command A** (mentioned in Chapter 2). **Stand one up first, swap as needed.**

## RAG — put pgvector to work

This is the payoff from Chapter 2. Turn internal documents, code, and mail into
...
```python
# 1) embed a document and put it in the Chapter 2 pgvector
...
The table for which Chapter 2 "only had the vessel ready" now gets its
...
Chapter 3 gate, beyond the reverse proxy.
...
stronger.** This is the same shape as the mail relay (Chapter 6) and
Cloudflare (Chapter 8).
...
- **RAG (pgvector)** — fill the Chapter 2 vessel with internal data, answer with citations
...
From Chapter 2 through Chapter 10, we replaced Microsoft 365 and the vendor
...
> As written at the start of Chapter 2 — **the effect of OSS is greater than
