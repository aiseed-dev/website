...
`pandoc -o report.pdf report.md --pdf-engine=xelatex` の **1 行で、学術論文・書籍品質の PDF** が生成できる。表紙、目次、フッター、引用、図表番号、ページ送り ── すべて自動。商業出版の体裁が、自分の手で作れる。

同じ Markdown 原稿から、Web 記事 ・ PDF レポート ・ EPUB 電子書籍 ・ ニュースレター ・ SNS 用画像 ・ プレゼンスライド ── **6 種類のメディアに同時展開**できる。書き直し不要、修正は Markdown 1 箇所。

社内ドキュメントを Markdown で整備すれば、Claude が「この方針について過去どんな議論があったか」を **5 秒で要約**する。組織の集合知が検索可能な財産になる。

英語論文を Markdown に変換して Claude に渡せば、**専門家レベルの要約と、自分の研究との接続点の指摘**が返る。研究者でなくても、最新の研究にアクセスできる。
