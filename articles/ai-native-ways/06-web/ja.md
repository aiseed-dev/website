...
作り直す時間がなければ、もっと速い手もある ── **今あるサイトを丸ごと静的に吸い出す**。ヘッドレスブラウザ(Playwright)で各ページの JavaScript を実行させ、描画後の HTML と、読み込む全ファイル(CSS・JS・画像・フォント)を保存し、参照を相対パスに書き換える。これを Cloudflare Pages に上げれば、動く WordPress を攻撃面ごと止められる。`requests` や `wget` は JavaScript を実行しないので取りこぼす ── ブラウザに描画させるのが要だ。Mac の SiteSucker(有料)と同じことが、AI に頼めば Python で無料で作れる(このサイトのリポジトリには `tools/mirror_site.py` がある)。
