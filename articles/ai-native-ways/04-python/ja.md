...
## 実例: 100 個の請求書 PDF から金額を抽出する

毎月、取引先から届く 100 個の請求書 PDF から金額を抜き出して合計する作業。手作業で 4 時間。これを Claude に書かせて 30 秒で終わらせる。

**手順 1: Claude に頼む**

```
あなた: invoices/ にある 100 個の請求書 PDF から、それぞれ
        「合計金額(円)」を抽出して、CSV(filename, amount)で
        出してくれる Python を書いて。日本語の PDF を想定。
```
...
`amounts.csv` に 100 行のデータが入っている。**処理時間 約 30 秒**。

**手順 3: エラーがあったら、Claude に直してもらう**

例: 一部の PDF で「合計」の表現が違って抽出に失敗した場合。

```bash
python3 extract.py 2>&1 | claude -p \
  "このエラーを直して。請求書フォーマットが複数ある可能性"
```

修正版コードが返る。「ご請求金額」「合計金額」「ご請求合計」── 複数の表現に対応した正規表現に書き直される。
...
**翌月も翌々月も、`extract.py` を実行するだけ**。書き直しはゼロ。「毎月 4 時間の作業」が「毎月 30 秒のコマンド」になる。

**手順 5: 一段階先 ── 異常検知**

```
あなた: amounts.csv の 12 ヶ月分を読んで、過去平均から 30%
        以上ズレている請求を「要確認」として抽出する Python を書いて
```

15 行の Python が返る。**経理担当が見落としていた請求ミスが見つかる**。これは Excel では絶対できない。

書く能力は要らなかった。**使う能力**だけで、4 時間の手作業が、30 秒の信頼できる処理に変わる。
