...
description: Drives the "ソフトウェア開発編" sub-series of "AIネイティブな仕事の作法" — 23 chapters in three 編 (parts, each numbered from 1: 導入編/自立編/転換編) that argue the SIer-commissioned development model is structurally obsolete and will be replaced in the near future by builder-led, AI-native development (irreversible because the economic and security premises have inverted, not because of a fixed completion horizon). The sub-series lives in `articles/ai-native-ways/software/` and renumbers from chapter 1. Use when adding any chapter in this sub-series, or when verifying that the sub-series stays internally consistent. Pair with authoring-aiways-chapter (mechanical scaffold) and writing-aiways-voice (prose conventions).
---

# Building the "ソフトウェア開発編" sub-series

This sub-series sits **inside** `articles/ai-native-ways/` but lives in its own subdirectory `software/` (global folders 01–23) and its own series-index page. It is organized into **three 編, each numbered from 1** (導入編 1–5 / 自立編 1–11 / 転換編 1–7). It shares the chapter template, frontmatter schema, voice, and accent palette with the parent series — what makes it a sub-series is the dedicated folder, the dedicated index page, and the **thesis arc**.
...
| **自立編 ── M365・Copilot・WordPress・基幹・GitHub から自立する** (part 2) | 第1–11章 / 06–16 | Overview/map (1: Microsoft と Google からの自立 — the bundle, the one-to-one mapping, no build steps) + hands-on build guide (2–11): replace the vendor products with OSS, one stack on the previous, then **prepare the information** (2-10: OCR/classification/codifying tacit knowledge — "整備こそ本体、AI は最後の一手") before mounting AI (2-11). "書くのではなく、立てる"; "the effect of OSS > the effect of AI." |
| **転換編 ── 産業構造の帰結** (part 3) | 第1–7章 / 17–23 | The structural consequence: why the SIer-commissioned model is now obsolete. |
...
- `prev_slug` / `next_slug` chain **inside** the sub-series only, across all **23** chapters in global folder order: folder 01's `prev_*` is empty, folder 23's `next_*` is empty. The chain runs continuously across the three 編 (…導入編5 `customer-codev` → 自立編1 `independence` → 自立編2 `foundation` → … → 自立編9 `fastapi` → 自立編10 `structure-knowledge` → 自立編11 `ai` → 転換編1 `two-worlds` → 転換編2 `sovereignty` → 転換編3 `sier-uneconomic` …). It does not cross into the parent series. Re-run the chain over all folders after adding/moving a chapter.

## Thesis arc

The thesis arc is carried by the 導入編 (steps 1–5, folders 01–05) and the 転換編 (steps 6–10, folders 17–23, the part-3 thesis chapters being 3-03…3-07). The 自立編 (folders 06–16) sits between them as the hands-on proof that step 5's "build it yourself" is materially true.
...
5. (Ch 05) Customers themselves become builders — but the first move is OSS, not code. Generic → use proven OSS (most economical, also more secure); personal → customize OSS in dialogue with AI (worked examples = parent series individual track); organization → stand up a generic foundation with OSS replacing M365/Copilot/WordPress (the 自立編). Only the company-specific logic is written with AI. What AI cannot do, the SIer cannot do either.
   — *自立編 (folders 06–16): the customer actually stands up the OSS stack (data → auth → documents → code → mail → meetings → web → API → prepare-the-information → AI).*
6. (3-03) The SIer-commissioned model becomes structurally uneconomic — commissioning de-responsibilizes and hollows out the customer's ability to use software; the build–test loop only in-house+AI can run; the SIer-model's disappearance is inevitable.
7. (3-04) Lock-in is the SIer model's true product — proprietary frameworks, abstraction layers, human dependency. Palantir's FDE model is examined as the archetype.
8. (3-05) Companies will bring builders in-house as **management — the CIO (Chief Information Officer) class**, not as a professional staff tier. The professional/practitioner work (lawyer/doctor-level practice) is done by AI; the human makes business decisions (IT judgment = business judgment = management judgment), with responsibility heavier than today's CIO.
9. (3-06) Japan's multi-tier subcontracting structure paradoxically enables the transition — primes can detach subcontractor agreements without firing anyone.
10. (3-07) The main changes arrive in the near future; because the economic and security premises have already inverted (3-01 parallel worlds, 3-02 sovereignty), the transition does not reverse.
...
**自立編 ── M365・Copilot・WordPress・基幹システム・GitHub から自立する (part "2", 第1–11章 / folders 06–16)** — 第1章 is the overview/map (no build steps); 第2–9章 are the install-guide register, each standing up one OSS stack, replacing a vendor product; 第10章 is the **information-preparation** step (OCR/classification/codifying tacit knowledge — no vendor replacement, the no-regret "整備こそ本体" investment); 第11章 mounts the local AI on the prepared information. The 自立編 replaces **both Microsoft 365 AND Google Workspace** (the stack is an alternative to both suites).
...
| 10 | `15-structure-knowledge` | 社内情報を整える ── 整備こそ本体、AI は最後の一手 | `structure-knowledge` | **no vendor replacement** — prepare the information *before* AI: OCR (Tesseract/ocrmypdf/vision model), classification & structuring (one place + metadata + Markdown), codifying tacit knowledge (interview → AI draft → ground confirms, same as 2-09). "整備こそ本体、AI は最後の一手"; a no-regret investment recoverable even without AI (ends personnel lock-in). Output to 2-05 files + 2-02 pgvector. |
| 11 | `16-ai` | 自前の AI を据える ── LLM と RAG | `ai` | **Copilot** (Gemini); Ollama/vLLM + RAG on pgvector. Starts the stack from **North Mini Code** (Cohere, open-weight Apache 2.0, local); forward note: Ryzen AI Max PRO 400 + Command A+ for in-house document RAG |

**転換編 ── 産業構造の帰結 (part "3", 第1–7章 / folders 17–23)**

| 第n章 | folder | title (JA) | slug | hinge claim |
|---|---|---|---|---|
| 1 | `17-two-worlds` | 企業は自分でコードを書かない ── 事務と基幹、二つの世界の並立 | `two-worlds` | **premise of the 転換編**: in-house dev was inefficient → office bought (Microsoft) + core outsourced (SIer) = two parallel separately-locked worlds + double tax; AI inverts the efficiency → both collapse together |
| 2 | `18-sovereignty` | デジタル主権 ── Microsoft 問題と Trump 問題 | `sovereignty` | **office-side consequence**: M365 now worse on cost AND security — the Microsoft problem (US CLOUD Act, per-seat rent, remote kill-switch) + the Trump problem (US-government dependence is a weaponizable lever; can't be trusted); OSS + sovereign local-weight AI is the cheaper-and-safer default |
| 3 | `19-sier-uneconomic` | SIer委託モデルの構造的不経済 | `sier-uneconomic` | commissioning de-responsibilizes and hollows out the ability to use software; the build–test loop only in-house+AI can run; SIer-model disappearance is inevitable |
| 4 | `20-lockin` | ロックイン問題 | `lockin` | Palantir FDE archetype |
| 5 | `21-hiring-builders` | 各社がビルダーを雇用する時代 | `hiring-builders` | builders as management — the CIO class (professional work is AI's) |
| 6 | `22-japan-transition` | 日本のSIer業界の転換と雇用流動性 | `japan-transition` | multi-tier subcontracting enables shift |
| 7 | `23-five-years` | もう戻らない構造転換 | `five-years` | irreversible (premises inverted), near-term horizon |
...
6. Add a Mermaid `flowchart` only where it earns its place — the structural-change argument lends itself to diagrams in 1-02, 1-04, and the 転換編 (e.g. 3-01, 3-03). 導入編 chapters can use one to show how a stack sits on the previous. Other chapters may go without.
7. Update prev/next links on the neighboring chapters **within the sub-series only** (both languages). The first folder (01) keeps empty `prev_*`; the last folder (23, `five-years`) keeps empty `next_*`.
...
- **Numbers**: the order-of-magnitude price-gap claim no longer has a dedicated thesis chapter (the former price-gap chapter was removed). Chapters may cite concrete component prices (monthly subscriptions, project quotes) as evidence, but do not lean on a single headline ratio (e.g. "10–100×") as the thesis — let the structural argument carry it.
- **The irreversibility-from-inversion framing** belongs to 3-07 (five-years): the main changes arrive in the near future, and because the economic and security premises have inverted (3-01 parallel worlds, 3-02 sovereignty), the shift does not reverse. Do NOT make the old hard "5年で完了する" completion prediction; earlier chapters speak of "近い将来 / 数年で" only loosely, and the climactic near-future-plus-inversion framing is reserved for 3-07.
