...
*一、Debianが動いているPC。* 読んでいる間に手を動かすので、最後の章を終えたとき、あなたの主作業環境はDebianになっているはずだ。
...
時間は、章ごとに30分〜1時間を見ておけばいい。週末を二〜三回使えば終わる分量だ。
...
*One: a PC that runs Debian.* Because you move your hands as you read, by the time you finish the last chapter, your main working environment should be Debian.
...
Budget 30 minutes to 1 hour per chapter — a volume you can finish over two or three weekends.
...
link:/ai-native-software/[AI ネイティブなソフトウェア開発]で詳述したやり方。
...
them with LibreOffice / Thunderbird / Firefox-Chromium. *This book goes one step further: switch your own working surface to Markdown + CSV + Python* — the approach detailed in link:/en/ai-native-software/[AI-Native Software Development]
...
具体的なコード・実測値は link:/ai-native-software/[AI ネイティブなソフトウェア開発]
...
Concrete code and measurements live in link:/en/ai-native-software/[AI-Native Software Development]
...
* *可能な限り、npm を使わない選択肢を検討する*:本書(と
  link:/ai-native-software/web-build/[2-10「Web を作る」])
...
それは link:/ai-native-software/[AI ネイティブなソフトウェア開発] に書いた通りで、
...
The book's claim is simple — *before pushing an OS swap, change the tools first*. This is exactly what link:/en/ai-native-software/[AI-Native Software Development]
...
*シリーズ完結にあたって：* この教科書全体はlink:/claude-debian/[Claudeと一緒に学ぶDebian 一覧]から辿れる。最後まで読み通していただき、ありがとうございました。
...
*On the completion of the series:* The whole textbook can be navigated from link:/en/claude-debian/[Learning Debian with Claude — All chapters]. Thank you for reading it through to the end.
