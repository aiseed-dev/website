...
Word 起動時間: 3〜10 秒。VS Code で `*.md` を開く時間: 0.5 秒。1 日 30 回開くと、年間 30 時間以上の節約。

Word ファイルの token 消費: 5,000 文字で約 8,000 トークン(書式情報を含むため水増しされる)。同じ内容の Markdown は約 4,000 トークン。**Claude に渡し続けるなら、書式は捨てるほどコストが下がる**。

## 実例: 12 ヶ月分の議事録を 30 分で Markdown 化する

過去 12 ヶ月分の Word 議事録(各 30 KB、合計 360 KB)を、検索・分析・PDF 化できる状態に変える。
...
12 ファイルが 5 秒で `.md` に変換される。Word の書式は剥がれ、見出し・段落・リストの構造だけが残る。

**手順 2: Claude に整形させる**

```bash
cat minutes-md/2026-04.md | claude -p \
  "この議事録を、決定事項・保留事項・アクションアイテムに分けて整形して"
```

冗長な前置きが削られ、決定/保留/担当者が見出しに分かれた整理された Markdown が返る。

**手順 3: 過去 1 年から特定テーマを抽出**

```bash
grep -A 3 "肥料価格" minutes-md/*.md
```

過去 12 ヶ月で「肥料価格」が議論された箇所が、文脈 3 行付きで一気に出てくる。Word では VBA で 30 分かかる作業。
...
ヒラギノ明朝で組版された、目次つきの PDF が出てくる。**書籍出版社が出すレベルの体裁**。これを取引先に送る。

**手順 5: 12 ヶ月の議論パターンを AI に分析させる**
...
組織が同じ議論を何度も繰り返している構造的問題が、5 秒で見える。**Word では絶対に得られない洞察**。

ここまでの全工程、所要時間 30 分。Word に閉じ込めていた知識が、検索可能・AI 分析可能・印刷品質配布可能な財産に変わる。
