...
description: 「壊すな・触るな」はもう古い。AI で基幹システムの書き換えコストは 10 分の 1 になった。新しい AI ネイティブなシステムを FastAPI で作り、旧システムと並行稼働させ、出力を実測で突き合わせる。差分が消えたら旧を殺す。業務知識は一気に Markdown に出し、現場がテストを書き、委託は止める。新しいロジックは第2章の PostgreSQL を読み書きし、第3章の門番のトークンで本人確認する。参考実装は kura。
date: 2026.07.15
...
（文書を自分の側に取り戻す話は 第5章 で扱った。基幹の業務知識もまた、同じく
読める素材に落ちる。）
...
DDL の方言変換、Azure SQL や pgloader を使った具体的な移行手順は、**第2章** で
...
画面ごとに書き散らさず、**一箇所にまとめる**ためだ。公開Web(第8章)のフォームも、
...
して書き、現行と突き合わせて確かめる(導入編 第6章 VBA→Python と同じやり方だ)。
...
3. **API・画面層**: 現場用の出荷指示は **FastAPI**(本章)で API 化し、HTML で画面を
   出す。社内 LAN だけで動かすので第4章の miniPC でホスト可
...
これは第5章の「文書を自分の側に取り戻す」の **基幹システム版** だ。「Office を一度に
...
テストを書き、委託は止める。書き換えた基幹ロジックは、**第2章の DB・第3章の門番**に
