...
- **保存対象** — `GET` かつ `200` かつ非 HTML のみ。POST・リダイレクト・エラー
  応答で得られる資産は保存されない。
- **Next.js 等の RSC / クライアント遷移** — App Router は `--js` 時にリンクを
  プリフェッチし、ページと同名パス（`/contact` 等）へ RSC ペイロード
  （`text/x-component`）を取りに行く。これはページの重複なので**保存対象から除外**
  している（同じパスがファイルとディレクトリで衝突して落ちるのを防ぐため）。
  なお RSC を静的化しないので、**クリックによるクライアント遷移は再現されない**。
  各ページの直リンク（`/contact` → `contact/index.html`）は機能するので実用上は
  問題ないことが多い。パス衝突自体は `save_bytes` がスキップして耐えるが、
  取りこぼしが疑わしいときは標準エラーを確認する。
