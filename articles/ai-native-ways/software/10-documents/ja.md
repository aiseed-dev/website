...
2-03で、認証を持つ **PocketBase** をもう立てた。文書の置き場は、**それを
門番にして、あとはファイルで足りる**。
...
まま持つ ── そのほうが、後で機械(Polars・AI)がそのまま読める。**実体は
ファイル**、この一点は変えない。
...
- **xattr(依存最小)** ── 別ストアを一切足したくないなら、ファイル自身の拡張
  属性に貼る。ただし xattr は **`cp`・`rsync`・`tar`・バックアップで静かに
  落ちる**(`-a` / `-X` / `--xattrs` が要る)── 権限が消える事故になり得るので、
...
権限のためだけに重い別アプリ(Nextcloud)は要らない。**門番が身元を、
構造化ストアが権限を**持つ ── 役割を分ける。
...
ここが「独自に組む」最大の利点だ。**認証は2-03の門番がそのまま効き、
権限は手元の構造化ストア(または xattr)が持つ**ので、Nextcloud のような
...
取引先との共有に、専用の共有基盤は要らない。**社外に渡すものは、出口から
出す** ── 元ファイルではなく、出口の形式(PDF / .docx)に変換して、メール
...
ここで2-02とつながる。**人は OnlyOffice(Excel 形式)で入れて読み、
